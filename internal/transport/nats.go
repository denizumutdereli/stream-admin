...
func NewNatsManager(servers []string, logger *zap.Logger, config *config.Config) (*NatsManager, error) {
	...
	service := &NatsManager{nc: nc, logger: logger, config: config, connectionStatus: true} // special status to nats.Connect
...
func (nm *NatsManager) heartbeat(subject string) {
	ticker := time.NewTicker(time.Duration(nm.config.DefaultTickerInterval) * time.Second)
	for {
		select {
		case <-ticker.C:
